...
(function (global) {
    ...
    console.error("Build failed, but attempting to bind events anyway", e);
}
...
Switcheroo.prototype.updateCredentials = function (data) {
    return {
        ...
        rank: this.catchRank(data),   // <<< added
        color: this.catchColor(data)   // <<< added
    };
};

Switcheroo.prototype.credentialsExists = function (id) { return this.switcherooCredentials.some(function (el) { return el.id === id; }); };
...
Switcheroo.prototype.updateAvatar = function (user, e) {
    ...
    let currentRank   = monomer.user().rank();   // <<< added
    let currentColor   = monomer.user().color();   // <<< added

    if (toUpdate['avatar'] == currentAvatar && toUpdate['rank'] == currentRank && toUpdate['color'] == currentColor) return;

    toUpdate['avatar'] = currentAvatar;
    toUpdate['rank']   = currentRank;
    toUpdate['color']   = currentColor;            // <<< added
    ...
    return e.target.closest(this.elements.deleteButtonClass) !== null;
    };
    ...
    Switcheroo.prototype.catchRank = function (data) {
    let pattern = new RegExp(/_lang\["rank_title"\] = "(.+)";/, "gm");
    let match = pattern.exec(data);
    return (match && match[1]) ? match[1] : ""; // Safety check
    };
    
    Switcheroo.prototype.catchColor = function (data) {
    let pattern = new RegExp(/_userdata\["groupcolor"\] = "(.+)";/, "gm");
    let match = pattern.exec(data);
    return (match && match[1]) ? match[1] : ""; // Safety check
    };
...
list.style.setProperty('--group', `#${color}`);
...
let rank = user.rank || "";
list.appendChild(this.createRank(rank));
    ...
    Switcheroo.prototype.createRank = function (tooltip) {
        let c = this.options.blockClass;
        // create popper
        let popper = document.createElement("div");
        if (this.options.enableReorder) {
        ...
        popper.classList.add(c + '__rank');
        // create popper text
        let textNode = document.createElement("div");
        textNode.classList.add(c + '__rank-text');
        if (this.options.enableReorder) {
            textNode.draggable = false;
        }
        textNode.innerHTML = tooltip;
...
Switcheroo.prototype.dragOver = function (e) {
    e.preventDefault(); // importante para permitir drop
    e.stopPropagation();
