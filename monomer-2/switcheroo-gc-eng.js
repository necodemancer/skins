...
Switcheroo.prototype.updateCredentials = function (data) {
    return {
        ...
        groupcolor: this.catchColor(data)   // <<< added
    };
};
...
Switcheroo.prototype.updateAvatar = function (user, e) {
    ...
    let currentColor   = monomer.user().groupcolor();   // <<< added

    if (toUpdate['avatar'] == currentAvatar && toUpdate['rank'] == currentRank && toUpdate['groupcolor'] == currentColor) return;

    toUpdate['avatar'] = currentAvatar;
    toUpdate['rank']   = currentRank;            // <<< added
    toUpdate['groupcolor']   = currentColor;            // <<< added
...
if (user.groupcolor) {
let groupcolor = "--group:#"+user.groupcolor+";";
list.setAttribute("style", groupcolor);
}
